...
from .models import StudentLeaveApplication, FacultyLeaveApplication,ExtendedUser
from .forms import StudentLeaveApplicationForm, FacultyLeaveApplicationForm
...
def student_leave_list(request):
    leaves = StudentLeaveApplication.objects.all()
    return render(request, 'adminApp/student_leave_list.html', {'leaves': leaves})

def approve_student_leave(request, leave_id):
    leave = get_object_or_404(StudentLeaveApplication, id=leave_id)
...
def faculty_leave_list(request):
    leaves = FacultyLeaveApplication.objects.all()
    return render(request, 'adminApp/faculty_leave_list.html', {'leaves': leaves})

def approve_faculty_leave(request, leave_id):
    leave = get_object_or_404(FacultyLeaveApplication, id=leave_id)
    leave.status = 'Approved'
    leave.save()
    messages.success(request, "Faculty leave approved successfully.")
    return redirect('faculty_leave_list')

def reject_faculty_leave(request, leave_id):
    leave = get_object_or_404(FacultyLeaveApplication, id=leave_id)
    leave.status = 'Rejected'
    leave.save()
    messages.error(request, "Faculty leave rejected.")
    return redirect('faculty_leave_list')

def manage_leave(request):
    return render(request, 'adminApp/manage_leave.html')

def approve_leave(request, leave_id):
    # Try to find leave in StudentLeaveApplication
    leave = StudentLeaveApplication.objects.filter(id=leave_id).first()

    # If not found, check FacultyLeaveApplication
    if not leave:
        leave = FacultyLeaveApplication.objects.filter(id=leave_id).first()
